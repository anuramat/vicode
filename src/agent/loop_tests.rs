...
/// a non-inheriting child starts like a primary agent: no parent messages,
/// instructions read from the context files of its own (frozen) workdir
#[tokio::test]
async fn fresh_spawn_reads_instructions_from_child_workdir() {
    use crate::agent::AgentId;
    use crate::tools::agent::spawn::SpawnArguments;
    use crate::tools::agent::spawn::SpawnCall;
    use crate::tools::agent::spawn::SpawnResult;
    ...
    let parent_workdir = agent.project.agent_workdir(&agent.id);
    tokio::fs::create_dir_all(&parent_workdir).await.unwrap();
    // written after the parent loaded its instructions
    ...
    let state = agent.project.store().load_state(&child).await.unwrap();
    let history = &state.context.history;
    assert!(history.instructions().contains("FRESH-MARKER"));
    assert!(!format!("{:?}", history.state().messages).contains("PARENT-ONLY"));
}
