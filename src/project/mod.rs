...
impl Project {
    ...
    pub async fn instructions(
        ...
        let root = self.agent_workdir(aid);
...
mod tests {
    ...
    #[tokio::test]
    async fn instructions_read_context_files_from_agent_workdir() {
        let project = Project::new_test().unwrap().0;
        let aid = AgentId::from("ctx".to_string());
        project
            .new_agent_workdir(&project.head_commit(), &aid)
            .await
            .unwrap();
        tokio::fs::write(project.agent_workdir(&aid).join("AGENTS.md"), "MARKER")
            .await
            .unwrap();
        assert_eq!(
            project.instructions(&aid).await.unwrap(),
            format!("{}MARKER", *INSTRUCTIONS)
        );
    }
