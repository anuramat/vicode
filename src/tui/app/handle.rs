...
mod tests {
    ...
    async fn tab_history_replays_authoritative_history_updates_exactly() {
        ...
        let events = vec![
            HistoryUpdate::UserMessage(UserMessage::new_at("hi".into(), 1)),
            HistoryUpdate::GenerationIncremented,
            HistoryUpdate::TurnResponse(AssistantEvent::Created(2)),
            HistoryUpdate::TurnResponse(AssistantEvent::Started(3)),
            HistoryUpdate::TurnResponse(AssistantEvent::Delta(Delta {
                id: "out".into(),
                delta: DeltaContent::Output("hello".into()),
                received_at: 4,
            })),
            HistoryUpdate::TurnResponse(AssistantEvent::Completed {
                items: vec![],
                ended_at: 5,
            }),
        ];
